...
import '../bloc/question_list_bloc/bloc.dart';
import '../widget/drop_down_search_widget.dart';

class FormSurvey2OnlineScreen extends StatefulWidget {
  const FormSurvey2OnlineScreen({super.key, required this.taskList});
  final task.Data taskList;

  @override
  State<FormSurvey2OnlineScreen> createState() =>
      _FormSurvey2OnlineScreenState();
}

class _FormSurvey2OnlineScreenState extends State<FormSurvey2OnlineScreen> {
  late List<Data> data = [];
  late List<AnswerResultsModel> results = [];
  bool isLoading = true, isExpired = false;
  ...
  @override
  void initState() {
    questionListBloc.add(QuestionListAttempt(widget.taskList.code!));
    super.initState();
  }
  ...
  _sortingData() async {
    for (int i = 0; i < data.length; i++) {
      ...
      controllers[i].text = data[i].answer ?? " ";
    }
            ...
            InkWell(
              onTap: () {
                ...
                Navigator.pushNamed(
                    ...
                    .then((value) {
                  questionListBloc
                      .add(QuestionListAttempt(widget.taskList.code!));
                });
              },
              child: Container(
                width: double.infinity,
                                                                  ...
                                                                  : (values) {
                                                                      setState(
                                                                        ...
                                                                        for (int i =
                                                                            ...
                                                                            i++) {
                                                                          if (data[index].answerChoice![i].questionOptionDesc ==
                                                                              values) {
                                                                            if (results.isEmpty) {
                                                                              results.add(AnswerResultsModel(pAnswer: data[index].answer, pAnswerChoiceId: data[index].answerChoice![i].id, pCode: data[index].code));
                                                                            } else {
                                                                              var isData = results.where((element) => element.pCode!.contains(data[index].answerChoice![i].questionCode!));

                                                                              if (isData.isNotEmpty) {
                                                                                results.removeWhere((element) => element.pCode!.contains(data[index].answerChoice![i].questionCode!));
                                                                                results.add(AnswerResultsModel(pAnswer: data[index].answer, pAnswerChoiceId: data[index].answerChoice![i].id, pCode: data[index].code));
                                                                              } else {
                                                                                results.add(AnswerResultsModel(pAnswer: data[index].answer, pAnswerChoiceId: data[index].answerChoice![i].id, pCode: data[index].code));
                                                                              }
