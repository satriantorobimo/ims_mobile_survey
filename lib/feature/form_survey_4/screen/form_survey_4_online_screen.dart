...
import '../bloc/reference_list_bloc/bloc.dart';
import '../data/hubungan_model.dart';
import '../provider/form_survey_4_provider.dart';

class FormSurvey4OnlineScreen extends StatefulWidget {
  const FormSurvey4OnlineScreen({super.key, required this.argsSubmitDataModel});
  final ArgsSubmitDataModel argsSubmitDataModel;

  @override
  State<FormSurvey4OnlineScreen> createState() =>
      _FormSurvey4OnlineScreenState();
}

class _FormSurvey4OnlineScreenState extends State<FormSurvey4OnlineScreen>
  ...
  void initState() {
    ...
    if (widget.argsSubmitDataModel.taskList.status == 'WAITING' ||
      ...
      referencelistBloc
          .add(ReferenceListAttempt(widget.argsSubmitDataModel.taskList.code!));
    } else {
      setState(() {
        isLoading = false;
      });
  ...
  void _sumberInformasiDetailView(String name, String phone, String phoneArea,
      String remark, String nilai, int index, bool isEdit) {
  ...
  Future<void> _getData(List<Data> data) async {
            ...
            InkWell(
              onTap: () {
                ...
                Navigator.pushNamed(
                    ...
                    .then((value) => referencelistBloc.add(ReferenceListAttempt(
              ...
              listeners: [
                BlocListener(
                  ...
                  listener: (_, ReferenceListState state) async {
                    if (state is ReferenceListLoading) {
                      _loadingData(context);
                    }
                    if (state is ReferenceListLoaded) {
                      await _getData(state.referenceListResponseModel.data!);
